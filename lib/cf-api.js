/**
 * Cloudflare API Client for SitesPro
...
export async function bindPagesDomain(apiToken, accountId, projectName, domainName) {
  ...
  const data = await handleResponse(res, 'Failed to bind domain to Pages project');
  return data.result;
}
...
export async function setupEmailForwarding(apiToken, zoneId, targetDomain, forwardingEmail) {
  ...
  // B. 创建转发规则：info@targetDomain -> forwardingEmail
  const rulesUrl = `https://api.cloudflare.com/client/v4/zones/${zoneId}/email/routing/rules`;
  const rulePayload = {
    name: 'SitesPro Auto Forward',
    enabled: true,
    matchers: [
      {
        type: 'literal',
        field: 'to',
        value: `info@${targetDomain}`,
      },
    ],
    actions: [
      {
  ...
  const data = await handleResponse(res, 'Failed to setup Email Routing forward rule');
  return data.result;
}
